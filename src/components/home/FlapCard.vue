...
export default {
  ...
  methods: {
    ...
    flapCardRotate () {
      ...
      // 背面左半圆转动至90度时才开始背景颜色至的变化
      if (backFlapCard.rotateDegree < 90) {
        backFlapCard._g += 5
      }
      // 当正面右半圆和背面左半圆相交时，让背面坐半圆覆盖正面右半圆
    ...
    startFlapCardAnimation () {
      ...
      this.task = setInterval(() => {
        // 动画原理
        // 1: 图片分左半圆和右半圆，转动时分正面(前面一张)和背面(后面一张)
        // 2: 每次动画开始时,背面左半圆先沿右Y轴线转动到180deg的位置,并设定其背景颜色
        // 3: 动画开始后正面右半圆和背面左半圆同时进行转动,同时改变背景颜色
        // 4: 达到90deg的位置时，让背面右半圆z-index增加覆盖正面左半圆后继续转动至完成一轮动画
        this.flapCardRotate()
      }, this.intervalTime)
    },
    startPointAnimation () {
